...
- i18n ключи в Zod `.meta()` + глобальный `z.config({ customError })` дают полную локализацию форм без изменения компонентов
- Русские plural forms (символ/символа/символов) работают через ICU Message Format в next-intl
- RTL-языки (арабский, иврит) поддерживаются нативно через Chakra UI v3 direction — все поля автоматически зеркалятся

**Кому полезно:**

- Junior: понять как переводить label, placeholder и ошибки валидации через i18n ключи в `.meta()`
- Middle: освоить глобальный error map для Zod, plural forms (ICU) и перевод options в Select/RadioGroup
- Senior: оценить архитектуру RTL-поддержки, мгновенного переключения языка и масштабирование на 120+ строк переводов
