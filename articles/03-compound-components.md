...
В следующей статье — подробный обзор всех 50+ field-компонентов: от простого `String` до `CreditCard` (платёжная форма) и `MatrixChoice` (опросники).

---

_Это третья статья из цикла «@letar/forms — от боли к декларативным формам». [Предыдущая: Zod .meta()](02-zod-meta-single-source.md) | [Следующая: 50+ полей](04-40-fields.md)._
