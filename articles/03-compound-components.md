# Compound Components vs конфиг-объекты: архитектура form-библиотеки
...
Базовый `Form` покрывает 40 полей. Но что, если вашему приложению нужны кастомные?
...
- **Живой пример:** [forms-example.letar.best/examples/basic](https://forms-example.letar.best/examples/basic)
- **Все поля:** [forms-example.letar.best/examples/all-fields](https://forms-example.letar.best/examples/all-fields)
- **Исходный код:** [basic](https://github.com/kamiletar/letar-forms-example/blob/main/src/app/examples/basic/page.tsx) | [all-fields](https://github.com/kamiletar/letar-forms-example/blob/main/src/app/examples/all-fields/page.tsx)
- **Клонировать:** `git clone https://github.com/kamiletar/letar-forms-example && cd letar-forms-example && npm install && npm run dev`

В следующей статье — подробный обзор всех 40 field-компонентов: от простого `String` до `Schedule` (редактор недельного расписания) и `RichText` (WYSIWYG на Tiptap).

---

_Это третья статья из цикла «@letar/forms — от боли к декларативным формам». [Предыдущая: Zod .meta()](02-zod-meta-single-source.md) | [Следующая: 40 полей](04-40-fields.md)._
